...
src = 'C:/iloom-workspace/10-projects/12-amoeba-tk/amoeba monthly/2025년 일룸 사업부 분개장.xlsx'
out = 'C:/iloom-workspace/10-projects/12-amoeba-tk/amoeba monthly/2025년 일룸 사업부 분개장_처리완료.xlsx'

print("파일 읽는 중...")
df = pd.read_excel(src)
print(f"원본: {len(df)}행, {len(df.columns)}열")

df['채산계정3_원본'] = df['채산계정3']

# 1. 직접간접
keywords_direct = [
    '유통경쟁력강화팀','온라인CX개선팀','리테일','직영+리테일','영업효율','일룸유통',
    '일룸 온라인(공식몰)','일룸 온라인(공통)','일룸 온라인(외부몰)',
    '일룸 직영(공통)','일룸 직영-강동아이파크',
    '직영사업팀(공통)','직영사업팀-노원','직영사업팀-논현','직영사업팀-대구',
    '직영사업팀-대전둔산','직영사업팀-마포서대문','직영사업팀-부산센텀',
    '직영사업팀-분당서현','직영사업팀-송파','직영사업팀-수원광교','직영사업팀-용산',
]
def 직접간접분류(val):
...
print(f'팀세부 분류: {df["팀세부"].value_counts().to_dict()}')
print(f'저장: {out}')
