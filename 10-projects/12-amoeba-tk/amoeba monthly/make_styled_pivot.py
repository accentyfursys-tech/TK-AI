...
FILE_IN  = 'C:/iloom-workspace/10-projects/12-amoeba-tk/amoeba monthly/02. 분개장정보상세_처리완료.xlsx'
FILE_OUT = FILE_IN

df = pd.read_excel(FILE_IN, sheet_name='RAW')

# 열 순서: 리테일/직영/온라인 각각 간접/직접
팀순서 = ['리테일', '직영', '온라인']
직접간접순서 = ['간접', '직접']

# 피봇 계산: 채산계정3 × (팀세부, 직접간접) 합계
pivot = df.groupby(['채산계정1','채산계정2','채산계정3','팀세부','직접간접'])['금액(안분후)'].sum().reset_index()
...
def get_subtotal_grand(c1, c2):
    rows = pivot[(pivot['채산계정1']==c1) & (pivot['채산계정2']==c2)]
    return int(rows['금액(안분후)'].sum()) if len(rows) > 0 else 0

def get_c1total_grand(c1):
    rows = pivot[pivot['채산계정1']==c1]
    return int(rows['금액(안분후)'].sum()) if len(rows) > 0 else 0

def get_grandtotal_all():
    return int(pivot['금액(안분후)'].sum())
...
COLOR = {
    ...
    '변동비':     'FF00B0F0',  # 파랑
    '영업외손익': 'FFFF0000',  # 빨강
    'header':     'FF404040',  # 헤더 배경 (진회색)
    'header_sub': 'FF595959',  # 서브헤더
    'c2':         'FFEEEEEE',  # 채산계정2 행 배경
    'total':      'FFFFF2CC',  # 소계/합계 노랑
}
...
# 헤더 작성
# ============================================================
# 행1: 팀 헤더 (리테일, 직영, 온라인, 총합계)
# 행2: 간접/직접 헤더
# A열: 행 레이블 (채산계정3), B열 비워둠 (채산계정1/2 표시)

# 컬럼 배치: A=레이블, B~M=데이터(각팀 간접/직접/소계 = 3팀 x 3 = 9), N=총합계
# 팀별: 간접, 직접, 소계 → 총 3열씩 = 9열 + 총합계 1열 = 10 데이터열
# A(레이블) + 10데이터열 = 11열
...
def team_col_start(team_idx):
    return DATA_START + team_idx * 3  # 간접/직접/소계

# 총합계 컬럼
TOTAL_COL = DATA_START + len(팀순서) * 3

# 행1: 팀명 헤더 (2열씩 span + 소계 = 3열씩 merge)
...
def write_row(row, label, vals, fill_color=None, font=None, is_total=False):
...
total_vals = {(팀, di): int(pivot[(pivot['팀세부']==팀) & (pivot['직접간접']==di)]['금액(안분후)'].sum())
